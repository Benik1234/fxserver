	...
	if not playersProcessing[source] then
		...
		playersProcessing[_source] = ESX.SetTimeout(10000, function()
			...
			if xSpice.limit ~= -1 and (xSpice.count + 1) >= xSpice.limit then
				TriggerClientEvent('esx:showNotification', _source, _U('gold_processingfull'))
			elseif xCannabinoid.count < 1 then
				TriggerClientEvent('esx:showNotification', _source, _U('gold_processingenough'))
			else
				xPlayer.removeInventoryItem('gold_o', 10)
				xPlayer.addInventoryItem('gold_t', 1)
	
				TriggerClientEvent('esx:showNotification', _source, _U('gold_processed'))
			end
