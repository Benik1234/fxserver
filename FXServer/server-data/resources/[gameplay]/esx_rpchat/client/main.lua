...
AddEventHandler('esx_rpchat:sendLocalOOC', function(playerId, title, message, color)
	...
	if target == source then
		TriggerEvent('chat:addMessage', {
      template = '<div style="padding: 0.45vw; margin: 0.05vw; background-color: rgba(99, 99, 99, 0.6); border-radius: 10px;"><i class="fas fa-street-view"></i> {0}: {1}</div>',
        args = { title, message }
    })
	elseif GetDistanceBetweenCoords(sourceCoords, targetCoords, true) < 20 then
		TriggerEvent('chat:addMessage', {
...
Citizen.CreateThread(function()
	...
	TriggerEvent('chat:addSuggestion', '/ooc', 'Globálna OOC správa (uvidia všetci)',   { { name = _U('generic_argument_name'), help = _U('generic_argument_help') } } )
...
AddEventHandler('onResourceStop', function(resource)
	if resource == GetCurrentResourceName() then
		...
		TriggerEvent('chat:removeSuggestion', '/doa')
	end
end)
