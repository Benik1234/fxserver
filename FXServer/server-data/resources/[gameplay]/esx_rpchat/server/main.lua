...
RegisterCommand('ooc', function(source, args, rawCommand)
  ...
  TriggerClientEvent('chat:addMessage', -1, {
      template = '<div style="padding: 0.25vw 0.7vw; margin: 0.3vw 0.5vw 0.5vw 0; border-radius:15px; background-color: rgba(200,200,200, 0.80);"><strong style="font-size: 9.8pt;"><width="30" height="30" style="position: relative; left: -5px; top:7px;">{0}:</strong><p style="padding-top: .35vw">{1}</p></div>',
      args = { playerName, msg }
  })
end, false)
...
TriggerEvent("es:addGroupCommand", "announce", "admin", function(source, args, rawCommand)
 ...
 --  local msg = rawCommand:sub(6)
  ...
  TriggerClientEvent('chat:addMessage', -1, {
      template = '<div style="padding: 0.45vw; margin: 0.05vw; background-color: rgba(204, 0, 0, 0.9); border-radius: 10px;"><i class="fas fa-bullhorn"></i>  Oznámení: {0}</div>',
      args = { toSay }
  })
...
--   local msg = rawCommand:sub(3)
  ...
  if xPlayer.job.name ~= 'police' then
    TriggerClientEvent('chat:addMessage', -1, {
      template = '<div style="padding: 0.3vw 0.8vw; margin: 0.5vw 0.5vw 0.5vw 0; border-radius:10px; background-color: rgba(202,40,40, 0.6);"><strong style="font-size: 10pt;"><img src="https://stars.djmetla.eu/blackmarket.png" width="20" height="20" style="position: relative; left: -5px; top:3px;">Blackmarket správa:</strong><br><p style="padding-top: .3vw">{0}</p></div>',
        args = {toSay}
    })
  end
end, false)--]]
