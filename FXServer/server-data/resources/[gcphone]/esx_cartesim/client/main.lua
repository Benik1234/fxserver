...
function OpenSimMenu()
	...
	local elements2 = {}
	  ...
	  ESX.TriggerServerCallback('esx_cartesim:GetList', function(sim)
		  ...
		  {
			  title    = 'Sim Kart Listesi',
			  elements = elements,
		  },
	  function(data, menu)
  
		local elements2 = {
			{label = 'Kullan', value = 'sim_use'},
			{label = 'Ver', value = 'sim_give'},
		  	{label = 'Sil', value = 'sim_delete'}
		  }
  
		  ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'sim_change', {
			title    = "Sim Kartı ile ne yapmak istiyorsun?",
			elements = elements2,
		  ...
		  function(data2, menu2)
			...
			if data2.current.value == 'sim_use' then
				...
				ESX.ShowNotification("SIM kartını etkinleştirdiniz ~o~" .. data.current.value.number)
			...
			if data2.current.value == 'sim_delete' then
				...
				ESX.ShowNotification("Sim kartını attın ~o~" .. data.current.value.number)
				TriggerServerEvent('gcPhone:allUpdate')
			end
		...
		if data.current.value == 'open_phone' then
			...
			ESX.TriggerServerCallback('esx_cartesim:CheckSim', function(sim)
				if sim then
					...
					ESX.ShowNotification("SIM kartınız yok!")
				end
			end)
